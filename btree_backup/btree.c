...
insert_item_in_btnode(btree_node *dest_btnode, node *item) {
		...
		// Update children's parent of item.
		if (left(item))
				left(item)->parent = dest_btnode;
		if (right(item))
				right(item)->parent = dest_btnode;
}
...
create_btreenode(node *item, size_t order, size_t key_size) {
	    ...
	    bt_node->compare = cmp;
		if (item && left(item))
				left(item)->parent = bt_node;
		if (item && right(item))
				right(item)->parent = bt_node;
...
print_btreenode(btree_node *bt_node) {
		...
		if (!bt_node) {
				...
				// Print all the data in that bt_node first before
				// heading to children
				while (node_ref1) {
